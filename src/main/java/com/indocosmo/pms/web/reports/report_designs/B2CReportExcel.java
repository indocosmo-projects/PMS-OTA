...
public class B2CReportExcel extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request,
		...
		subheaderCellStyle1.setFillBackgroundColor(IndexedColors.GREEN.getIndex());
		...
		if (b2cReport.size()!=0) {
			...
			HSSFSheet sheet = workbook.createSheet("B2C");
			...
			sheet.setColumnWidth(3, 4000);
				...
				cellPax1.setCellValue(amountFormat.format(Double.valueOf(b2cData0.getTotalTaxableVal())));
				...
				for (int i = 0; i < b2cReport.size(); i++) {
					...
					cellRoomdata.setCellValue("32-Kerala");

					HSSFCell cellGuestData = detailRow.createCell(2);
					cellGuestData.setCellStyle(contentCellStyle2);
					cellGuestData.setCellValue(amountFormat.format(Double.valueOf(b2cData.getTotalTaxPc())));

				
					HSSFCell cellpax1 = detailRow.createCell(3);
					cellpax1.setCellStyle(numericCellStyle);
					cellpax1.setCellValue(amountFormat.format(Double.valueOf(b2cData.getInvoiceBaseAmount())));
		...
		headerResponse = headerResponse
				.concat(("B2C REPORT").toLowerCase().replaceAll("\\s", "") + stringToday + ".xls");
