...
public class DetailedSeparatedInvoice extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document doc, PdfWriter writer,
		...
		Font fontchk = FontFactory.getFont(FontFactory.HELVETICA, 10);
		...
		if (txn != null) {
			...
			Font fontHead1 = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 9);
			...
			cellchk.setPhrase(new Phrase("Ref No.         : ", fontHead));
			...
			cellchk.setPhrase(new Phrase("Ref Dt.          : ", fontHead));
			cellchk.setHorizontalAlignment(Element.ALIGN_RIGHT);
			tablechk.addCell(cellchk);
			lineCount++;
			...
			for (Transaction txnInvoice : txnListInvoice) {
				...
				if (txnInvoice.getAcc_mst_id() != 4 && txnInvoice.getAcc_mst_id() != 5
					...
					/*if(txnInvoice.getAcc_mst_code().equals("POS")) {
						String orderTimeVal= txnInvoice.getTxn_time();
						String[] orderTime = orderTimeVal.split(" ");
						String[] hrMin = orderTime[1].split(":");
						String accMstNameWithTime = txnInvoice.getAccMstName() +"(" + hrMin[0]+":"+hrMin[1] + " hrs)";
						cell.setHorizontalAlignment(Element.ALIGN_LEFT);
						cell.setPhrase(new Phrase(accMstNameWithTime, font2));
						tableNew.addCell(cell);
					}
					else {*/
						cell.setHorizontalAlignment(Element.ALIGN_LEFT);
